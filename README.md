# Antigravity Claude Proxy (Custom Fork)

> **Custom fork** with "Universal Waterfall" fallback strategy.
...
This fork includes the following changes from the upstream [antigravity-claude-proxy](https://github.com/badri-s2001/antigravity-claude-proxy):

### Universal Waterfall Fallback

Regardless of what model the client requests, the proxy:

1. **Forces all requests to start with** `claude-opus-4-5-thinking`
2. **Falls back through the chain** when rate-limited:
   - `claude-opus-4-5-thinking` → `gemini-3-pro-high` → `gemini-3-pro-image` → `gemini-3-flash`
3. **Ignores Gemini 2.5 models** entirely
4. **Fallback is enabled by default** - no `--fallback` flag needed

### Files Modified

- `src/constants.js` - Defines `UNIVERSAL_FALLBACK_CHAIN`
- `src/fallback-config.js` - Implements chain-walking logic
- `src/server.js` - Forces model override and enables fallback by default
- `src/cloudcode/message-handler.js` - Recursive fallback enabled
- `src/cloudcode/streaming-handler.js` - Recursive fallback enabled

### Customizing the Chain

Edit `src/constants.js` to change the priority order:

```javascript
export const UNIVERSAL_FALLBACK_CHAIN = [
    'claude-opus-4-5-thinking',  // Priority 1
    'gemini-3-pro-high',         // Priority 2
    'gemini-3-pro-image',        // Priority 3
    'gemini-3-flash'             // Priority 4 (last resort)
];
```
